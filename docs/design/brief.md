...
| D9 | Header (1a/2a) | `[mark] Illini Dash [↗] [⋯] [⚙]` — the mark and the name on the left, the controls on the right, and nothing between them (the pill went on 2026-09-19, D2). **The `+` went with it, later the same day**: "theres no need to have a + at the top right to add cuz theres already one at the bottom right", so the floating `+` is the only add and is drawn on every tab. `⋯` carries: Download .ics, Google Calendar, Courses (show/hide, replacing the chip strip), Appearance — **not** Open full view (its own icon) and **not** Settings ("theres alr a settings button"). The sync button moves to the footer. |
...
| D14 | Tweaks | Two per-device toggles in the theme panel, **both default off (Source names flipped 2026-09-19)**: **Class colour** (a 4px course-hue edge on rows; labelled "Urgency edge" until 2026-09-26, though it never showed urgency) and **Source names** (which site a deadline came from, on the standard row's meta line; the compact row never shows it). "There's just too much information being shown" — and where a deadline came from is on the deadline screen, next to the link that opens it. Both toggles stay, so either can be turned back on. `localStorage`, same pattern as the theme; `DEFAULT_TWEAKS` in `core/theme.ts` is unchanged and the flip is in the two UI readers. |
