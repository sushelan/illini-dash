...
**They were all pictures of a 404 page, and the run said otherwise.** Every
`*-dark.png` here — the five this section names as the store screenshots among them
— was a 1280x800 render of the words "not found", the shots harness's own 404 body,
committed since `0d41212`. `capture()` reported success on `existsSync(file)`, which a
file from an older run satisfies, so a capture that never wrote printed a filename like
any other. The target is deleted before the capture now, so a failure is a failure.
Regenerated 2026-09-22: 48 shots, none failed. **Before uploading, open the five.** A
harness that renders the real build is worth nothing if nobody looks at what came out —
the same lesson as the `?open=health` press that spent a day photographing the wrong
screen.
