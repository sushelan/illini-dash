...
2. **`fixtures/piazza/post.json`** — one `content.get` response. No parser for it exists
   (CLAUDE.md: no parser before its fixture). The request shape is written down in
   `postBodyRequest`, and `fetchPostBody` throws rather than returning an empty answer.
