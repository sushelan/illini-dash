...
## Corpus Check
- 266 files · ~990,267 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 20 file(s) not represented in the graph (top: .css 13, (none) 5, .woff2 2)

## Summary
- 3346 nodes · 8961 edges · 170 communities (159 shown, 11 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 874 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c85da310`
...
Cohesion: 0.12
Nodes (29): 8. Storage, Amendment (2026-09-19): the announcement is not the row's name, ALL_OBSERVERS, BACKOFF_MINUTES, FETCHED_SOURCES, foundAlreadyPast(), isObserverHealth(), isRecord() (+21 more)

### Community 1 - "ParseError"
Cohesion: 0.08
Nodes (45): House rules for parsers, The signed-out page, and the positive marker it made possible, isOlderThan(), parseGradescopeDateTime(), shortHash(), FieldResult, KeyGuard, LoggedOutOptions (+37 more)
...
Cohesion: 0.11
Nodes (34): M5 — a `set-due` outranks every later source date, for ever, with nothing on screen to say the source now disagrees, I06 · Show runner-assumed 23:59 times as assumed, I08 · Local Done check-off, separate from Hide, Theme: two sources can never say done, #sec-tidy: Hidden and Ticked off rows, applyRetention(), badgesOf(), buildItem() (+26 more)

### Community 7 - "types.ts"
Cohesion: 0.11
Nodes (26): linkedom, isNoEndMarker(), parsePrairieLearnScheduleDate(), courseInstanceIdFrom(), CreditCell, creditStillOpen(), CreditTier, deadlinesFromSchedule() (+18 more)

### Community 8 - "schedule.ts"
Cohesion: 0.13
Nodes (28): I13 · Reminder toasts with Open / Snooze / Done buttons, I38 · Coalesce catch-up reminder bursts, word by real remaining time, I41 · Morning toast instead of 2h lead for runner-invented times, Review outcome — steps 9–12 (16 findings, 13 code defects), Quiet hours, alarmName(), BOOKING_HOUR, deferPastQuietHours() (+20 more)

### Community 9 - "Z. Test files that pin popup behaviour"
Cohesion: 0.09
Nodes (48): Z. Test files that pin popup behaviour, Defect: finished work with a late window open appeared nowhere, isFinished(), itemTone, isItemDone(), isTickedDone(), clockOf(), creditPercentFor() (+40 more)
...
Cohesion: 0.11
Nodes (26): ask(), detectInOffscreen(), ensureOffscreenDocument(), parseGradescopeDashboard(), parseHtml(), parseSmartPhysicsCourses(), runAdapterInOffscreen(), CourseSummary (+18 more)

### Community 12 - "options.ts"
Cohesion: 0.06
Nodes (32): I57 · Term rollover: term dates in the registry, Expired section, currentTermCode(), AdapterEntry, adapterPagePath(), addSiteUrl, buildAdapter(), captureButton, captureUrl (+24 more)

### Community 13 - "Adapter registry (bundled + daily GitHub refresh)"
Cohesion: 0.15
Nodes (23): Open: course sites split across pages, Parallelism policy, Parser rule 3: never index cells positionally, Trace the path, not just the file, Worker rule 3: a value this code invented is not a value the source stated, Worker rule 5: log both branches of any decision the user will have to debug, Worker rule 7: live data is a source of truth the fixtures are not, Adapter.columns — header-driven column lookup (+15 more)

### Community 14 - "popup.ts"
Cohesion: 0.08
Nodes (45): House rules from the ZIP acceptance pass, 2026-09-19, The focus mechanism, as built, N. Attention view and suggestions, AttentionName, courseColours(), coursesIn(), examCount(), ViewName (+37 more)

### Community 15 - "theme-panel.ts"
Cohesion: 0.16
Nodes (25): DARK_CLASS, DEFAULT_MODE, DEFAULT_THEME, DEFAULT_TWEAKS, DESIGN, isModeName(), isThemeName(), MODE_KEY (+17 more)

### Community 16 - "UX plan for the store release"
Cohesion: 0.19
Nodes (21): Header health dots: grey/green/yellow/red, UX plan for the store release, B1: a sat exam is not Overdue, B2: primary button invisible in dark (--brand on brand page), B3: white on accent fails AA; --accent-ink #1a0d04, Button system: btn-primary/secondary/quiet/icon, Computed WCAG contrast table, Copy guide: names, states, verbs; nothing from a spec reaches the screen (+13 more)

### Community 17 - "prairietest.ts"
Cohesion: 0.14
Nodes (18): parseDateAttribute(), parseDateRangeAttribute(), extractCourseCodes(), textOf(), parseDashboard(), cardFor(), EMPTY_CARD, examKey() (+10 more)

### Community 18 - "canvas.ts"
Cohesion: 0.17
Nodes (20): Decision: Canvas concluded-course filter via include[]=term, Shared parser primitives (src/core/parsing.ts), isInstant(), sameOriginHttpsUrl(), CanvasCourse, courseMap(), coursesUrl(), currentTermCourses() (+12 more)
...
Cohesion: 0.06
Nodes (72): House rules for mutation checks, A survivor sometimes indicts the design, 4. The date grammar, Mutation rule 3: a survivor sometimes indicts the design, A survivor sometimes indicts the design, 4. The date grammar, 1. Counts, readClock() (+64 more)

### Community 21 - "support.js"
Cohesion: 0.06
Nodes (75): boot(), bundledBlob(), cdnScriptFor(), collectProps(), compileAttr(), compileTemplate(), contentKey(), createComponentFactory() (+67 more)

### Community 22 - "piazza.ts"
Cohesion: 0.05
Nodes (48): Evidence first, Honest completion, Illini UI acceptance, Six-stage execution, Piazza and Campuswire on the front, Amendment (2026-09-18): three ways one field cost a whole class, What the term rule is, and why `status` is not it, CAMPUSWIRE_ORIGIN (+40 more)

### Community 23 - "icon"
Cohesion: 0.14
Nodes (27): Y. Every DOM id and class the popup writes, END_OF_DAY_HEADING, LATE_HEADING, reservationVerified(), icon(), ICON_PATHS, bookingWindowText(), emptyNote() (+19 more)

### Community 24 - "Chrome Web Store listing draft (§9 G5)"
Cohesion: 0.06
Nodes (42): Store description (plain text), Daily reminder for CBTF exams open for booking, Not affiliated with UIUC, Instructure, Gradescope or PrairieLearn, No account, no password, no server, One entry per assignment across Gradescope and Canvas, Chrome Web Store listing draft (§9 G5), Category: Workflow & Planning, Data disclosure form: website content read locally, never transmitted (+34 more)

### Community 25 - "Review R1 — does every old popup feature exist in the new one?"
Cohesion: 0.33
Nodes (4): 1. Counts, 5. The ten fixes worth making, ranked, 6. One structural note, Review R1 — does every old popup feature exist in the new one?
...
Cohesion: 0.04
Nodes (59): Announcement fixtures, addDays(), ASSUMED_CLOCK, BADGE, BADGE_WORD, badgeIn(), BOUNDARY, CAL_MONTH (+51 more)

### Community 28 - "piazza.test.ts"
Cohesion: 0.05
Nodes (70): Amendment (2026-09-18, corrected 2026-09-19): the reader version, and posts read at their snippets, Amendment (2026-09-18, evening): the fetch plan after the trace, The feed's shapes, as captured, The two re-read lines now count the same thing, class-page.html — `GET https://piazza.com/class/<nid>` (signed in), class-page-signed-out.html — `GET https://piazza.com/class/<nid>` (signed OUT), feed.json — `POST https://piazza.com/logic/api?method=network.get_my_feed`, Piazza fixtures (+62 more)

### Community 29 - "makeRowsNavigable"
Cohesion: 0.28
Nodes (9): 3. Every non-PRESERVED item, DEGRADED (4), REPLACED-RECORDED (49), L3 — A row with no URL can never be focused, so back-from-a-screen loses focus on it., H. The row, makeRowsNavigable(), renderFilters(), rowsInView() (+1 more)

### Community 30 - "deadline.ts"
Cohesion: 0.21
Nodes (21): L2 — Opening a screen moves focus nowhere., L8 — `openedFrom` survives a screen the tab change discarded., postUrl(), assumedTimeNote(), closeScreen(), el(), factRow(), markScreen() (+13 more)
...
Cohesion: 0.08
Nodes (50): House rules for the worker and the loop, Worker rule 1: the service worker is the file the suite cannot reach, so keep it empty, Worker rule 4: every store writer goes through the queue, and the queue is re-entrant, Decisions worth not re-litigating, W. What lives in Options, not the popup, Defect: the store queue deadlocked, allAdapters(), applyObserver() (+42 more)

### Community 33 - "core/registry.ts"
Cohesion: 0.09
Nodes (25): BUILD_ID, EXTENSION_VERSION, SOURCE_ORIGIN, sourceForUrl(), ADAPTER_KINDS, compareVersions(), FIELDS_ADDED_IN_1_1, FIELDS_ADDED_IN_1_2 (+17 more)

### Community 34 - "overrides.test.ts"
Cohesion: 0.15
Nodes (27): HANDOFF 2026-09-13: the row menu receives no mouse events, UI rule 4: when a control does something asynchronous, say so on the control, UI rule 5: a synthetic .click() is not a press, UI rule 6: the preview pane's coordinates are not the page's, Row menu (⋯), applyOverride(), acceptSuggestion(), applyDueOverride() (+19 more)
...
Cohesion: 0.06
Nodes (62): And the deterministic proposer reads the whole page itself, Every group now says how much of it is dated, Running it without a browser, The inventory says which groups carry dates, and the search reads them, The retry names the groups that do carry dates, What a student is told now, adapterFromCandidate(), AdapterIdentity (+54 more)
...
Cohesion: 0.17
Nodes (16): UI rule 2: every send() from a page needs a .catch, Worker rule 8: a message from the worker is data from another build, not a typed object, Course rename override (overrides.courseNames), Defect: a worker on an older build killed the settings page, FieldKind, fill(), isRecord(), NormalizedOptionsState (+8 more)
...
### Community 42 - "vitest"
Cohesion: 0.12
Nodes (8): vitest, actionOutcome, UNEXPLAINED_REFUSAL, html, visible, { document, window }, globals, V1

### Community 43 - "health.ts"
Cohesion: 0.07
Nodes (61): 9.3 The four screens, Structural decisions, Notes on items classified PRESERVED that moved, B1 — `footerLine` has no caller. The footer that ships is a second copy, and it paints green over sources that were never read, L12 — `compactAgo` is dead, and D10's "synced 2m ago" is not what ships, C. Health pill and source popover, Defect: enabling a course site stayed 'Checking…' and wrote state ok by hand, actionFor() (+53 more)
...
Cohesion: 0.07
Nodes (64): Month — two drawings of one month, 4. Cross-worker seam checks, BROKEN (1), L5 — Two round trips with no popup-side log line, and one with no caption., M8 — every student-typed date is built in `SITE_TIMEZONE`; every view buckets it in the browser's zone, D. Banners, O. The editor (manual items), dayKey() (+56 more)
...
Cohesion: 0.15
Nodes (17): I02 · Exam-day card on PrairieTest rows (room, duration, format), I04 · Late / reduced-credit window stays live after dueAt, I07 · Reduced-credit ladder shown before the deadline passes, I27 · 'Can reminders reach you?' check and test-reminder button, I31 · Surface parser data-quality flags instead of dropping rows, I46 · 'Not used by you' source state for PrairieLearn / PrairieTest, I49 · Adapter date grammar matching real fa26 pages, I54 · Versioned store migrations with memberKey remapping (+9 more)
...
Cohesion: 0.19
Nodes (7): currentTermCourses(), deps(), fetchPage(), fixture(), gatedDeps(), PAGES, runAdapter()

### Community 50 - "dates.ts"
Cohesion: 0.15
Nodes (24): A survivor has three meanings — decide which before acting, Mutation rule 2: a survivor is untested, unreachable, or redundant, Parser rule 1: a bad value costs its field, a missing hook throws, Parser rule 4: guard duplicate sourceIds on one page, Parser rule 5: typeof x === 'string' is not validation, A survivor has three meanings — decide which before acting, RFC-3339, Review outcome — PrairieLearn (12 findings, all survived) (+16 more)
...
Cohesion: 0.05
Nodes (76): 6. Checked and clean, M8 — The setup screen's "Connected" chip outranks the source's current state., L11 — `"attention"` survives in the popup's view table, with a comment asserting it still has a tab, Important files to resume from, Interaction review: four product findings and one harness gap, The repair batch: 1–7 done, 8–10 running, E. Tabs and view state, F. Course filter chips (+68 more)

### Community 54 - "manual.ts"
Cohesion: 0.23
Nodes (19): ASSUMED_TIME, dedupeInput(), editManualItem(), fieldsOf(), instantOf(), KINDS, ManualInput, ManualItemError (+11 more)

### Community 55 - "send"
Cohesion: 0.27
Nodes (22): displayCourseLabel(), send(), adapterGroup(), adapterPageName(), adapterRow(), clearRemoval(), el(), gcalSection() (+14 more)
...
Cohesion: 0.05
Nodes (69): House rules for the on-device model, A selector the page has not got is refused before the runner, One session per attempt, and what `kErrorUnknown` meant, The context window, The schema requires what the validator will demand, The shape, What the student is told, attemptCount() (+61 more)
...
Cohesion: 0.15
Nodes (13): EmptyReason, PostBody, PostPayload, EXPECTED, FEED, ingest(), NO_OVERRIDES, PAGE (+5 more)

### Community 66 - "sourcesToRecheck"
Cohesion: 0.11
Nodes (26): Open: Coursera for the online CS courses, Parser rule 11: never signed in is not session expired, and both are needs_login, Parser rule 12: a signed-out marker must be absent from the healthy page, Parser rule 13: a per-student URL means a source, not an adapter, Parser rule 6: match markers exactly and scope them to the smallest element, Parser rule 8: login detection needs the HTTP status, Amendment §4.2/§3.1: Gradescope row is a button before submission and an a after, Defect: the debounce asked 'how long' instead of 'has anything happened' (+18 more)
...
### Community 68 - "RawItem"
Cohesion: 0.17
Nodes (17): 2. Data model, RetentionResult, QualityFlag, IngestInput, Overrides, RawItem, StoreV1, demoPost() (+9 more)
...
Cohesion: 0.25
Nodes (11): STUDENT_POST_ID, dateOrigin, HEADING, isStudentsOwn(), movedHeading(), OWN_TIME_NOTE, OWN_TIME_NOTE_ALL_DAY, SOURCE_TIME_NOTE (+3 more)
...
Cohesion: 0.08
Nodes (47): The row menu bug — found and fixed 2026-09-18, R-3 — DEGRADED. A tweak changed in the Appearance panel does not repaint the list until the panel is dismissed, 3. Findings, ranked, 5. R1's ten, re-checked, L10 — `SUGGESTION_SOURCE[suggestion.source]` is unguarded (worker rule 8's shape)., L1 — `needs-you.ts:108` writes `".menu-surface"` out by hand., L4 — The deadline screen's ⋯ does not toggle., L6 — The Appearance panel is a `role="dialog"` the keyboard cannot enter. (+39 more)
...
Cohesion: 0.22
Nodes (13): Parser rule 2: silent empty is the worst outcome, Tier 0a (2026-09-10, 13 items), Repo layout: worker is wiring, decisions live in core/, §0.5 Chrome only, §0.3 Parsers fail loudly, Per-source health indicator, Notifications (§7), Offscreen document (DOM_PARSER) (+5 more)

### Community 101 - ".now"
Cohesion: 0.49
Nodes (6): fetchAll(), NeedsLogin, syncGradescope(), syncPrairieLearn(), syncPrairieTest(), syncSmartPhysics()
...
Cohesion: 0.07
Nodes (22): Checking the three lines without a model, The attempt line says what the model emitted, The inventory sketches one row's insides, The manifest needs no new permission, The three shapes it may propose, When nothing is proposed: the on-device model, ref_node_fs, ref_vitest_config (+14 more)
...
Cohesion: 0.07
Nodes (31): 0. Is it even an adapter?, 1. The capture, 2. The schema, 3. Where the date comes from, 5. The fixture test (required before it ships), 6. Delivery, Adding a course-site adapter (§4.5), How the date is read out of the located text (+23 more)

### Community 112 - "Triaging a beta report"
Cohesion: 0.25
Nodes (7): 1. Quote the report verbatim, first, 2. Separate symptom from cause, and do not stop at the first cause, 3. Check the fixtures can even reach the state — they usually cannot, 4. Decide which log line would have answered it — and add it, 5. Gate failure, or ordinary fix?, 6. The PROGRESS.md entry, Triaging a beta report

### Community 113 - "normalize.ts"
Cohesion: 0.18
...
Cohesion: 0.31
Nodes (9): M13: open the first-run tab on install, Phase E: first run and install, loginsToOpen(), needsSetup(), opensOnInstall(), SETUP_SOURCES, setupProgress, setupRows() (+1 more)
...
Cohesion: 0.13
Nodes (22): Check it in the mode Sushi actually uses (dark), Sushi's time is the scarce resource, UI rule 1: a popup and the service worker have different consoles, Light/dark is the is-dark class, not a media query, Theme choice in localStorage (illini-dash.theme / illini-dash.mode), V. Theme and dark mode, Light or dark is a setting (is-dark class), menu (+14 more)

### Community 118 - "healthPill"
Cohesion: 0.24
Nodes (11): Decision 4: manual deadline entry, aggregator or planner, I03 · Toolbar badge: today's count, red ! when a source is broken, I16 · Honest status line and stale-data banner, I21 · Manual deadlines as a sixth 'manual' source, I47 · Per-adapter health state and N->0 guard per course site, Theme: health is honest only inside the popup, #page-sub: version, last check, sources answered, #banners: one slot, not three (+3 more)
...
Cohesion: 0.09
Nodes (36): ReadMention, ASSUMED_HOUR, CAMPUSWIRE_MATCH, classCodeFromPath(), isClassFeed(), ObservedPost, ObserverFacts, PageContext (+28 more)
...
### Community 143 - "Review R2 — new classes of defect in the redesigned popup"
Cohesion: 0.29
...
Cohesion: 0.32
Nodes (7): buildId, copyStatic(), observerOptions, options, refuseConflictMarkers(), setup(), watch
...
Cohesion: 0.25
Nodes (5): esbuild, ref_node_crypto, referenceItems(), globals, popup
...
### Community 156 - "Canvas source (§4.1, REST API)"
Cohesion: 0.20
Nodes (12): Mutation rule 1: verify the mutation applied, Parser rule 10: a test that passes against a wrong implementation is not a test, Worker rule 6: a mutation check proves a test is load-bearing, not that it pins the right requirement, Amendment §4.1/§5.3: Canvas course_code is an opaque slug, Amendment §4.1: no while(1); prefix on this deployment, Defect: a Gradescope course labelled stat_425_120248_268442, The one source with no login page (course sites), Canvas plannable_type → Kind mapping (+4 more)

### Community 159 - "Adapter"
Cohesion: 0.33
Nodes (3): ValidationResult, SyncDeps, Adapter
...
### Community 165 - "offscreen.html: DOMParser host for the service worker"
Cohesion: 0.50
...
### Community 167 - "Independent reviewers"
Cohesion: 0.50
...
### Community 169 - "Calendar export (§8.3)"
Cohesion: 0.83
...
## Knowledge Gaps
- **842 isolated node(s):** `watch`, `buildId`, `options`, `observerOptions`, `name` (+837 more)
...
- **What is the exact relationship between `--blink-settings=preferredColorScheme, not --force-dark-mode` and `npm run shots headless capture script`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `ParseError` connect `ParseError` to `What the two stages read, and the scorecard over the real feed`, `types.ts`, `Illini Dash — design as built`, `messages.ts`, `core/campuswire.ts`, `prairietest.ts`, `canvas.ts`, `site.ts`, `piazza.ts`, `announce.ts`, `piazza.test.ts`, `background.ts`, `detect.ts`, `sync.test.ts`, `dates.ts`, `author.ts`, `RawItem`, `suggest.ts`, `syncSites`, `skeleton.ts`, `sync.ts`, `Sync loop runSync (§6)`, `.now`, `normalize.ts`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Why does `vitest` connect `vitest` to `ParseError`, `calendar.ts`, `dedupe.ts`, `types.ts`, `schedule.ts`, `Z. Test files that pin popup behaviour`, `page-url.ts`, `core/campuswire.ts`, `popup.ts`, `theme-panel.ts`, `prairietest.ts`, `package.json`, `canvas.ts`, `site.ts`, `piazza.ts`, `icon`, `preview-acceptance.test.ts`, `queue.ts`, `piazza.test.ts`, `local-adapters.test.ts`, `core/registry.ts`, `course-colour.test.ts`, `gcal-auth.ts`, `overrides.test.ts`, `detect.ts`, `compat.ts`, `health.ts`, `table-grid.ts`, `gcal-client.ts`, `sync.test.ts`, `gate0.ts`, `gcal.ts`, `manual.ts`, `scrub.ts`, `tokens.test.ts`, `popup-draw.test.ts`, `author.ts`, `ics.ts`, `piazza-real.test.ts`, `RawItem`, `diagnostics.ts`, `skeleton.ts`, `provenance.test.ts`, `exams-verified.test.ts`, `ref_node_fs`, `normalize.ts`, `core/setup.ts`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `Item` connect `calendar.ts` to `store.ts`, `ParseError`, `dedupe.ts`, `types.ts`, `Illini Dash — design as built`, `Z. Test files that pin popup behaviour`, `schedule.ts`, `messages.ts`, `core/campuswire.ts`, `popup.ts`, `preview-acceptance.test.ts`, `icon`, `9. UI`, `announce.ts`, `piazza.test.ts`, `deadline.ts`, `background.ts`, `overrides.test.ts`, `health.ts`, `screens/editor.ts`, `gcal-client.ts`, `gcal.ts`, `shell.ts`, `popup-draw.test.ts`, `ics.ts`, `piazza-real.test.ts`, `RawItem`, `suggest.ts`, `provenance.test.ts`, `exams-verified.test.ts`, `ref_node_fs`, `Auto-merge rule (§5.3)`, `normalize.ts`?**
